...
def test_prompt_asks_for_poll_options_only_for_a_poll():
    poll_prompt = build_prompt(POLL, "subject", "title", "script")
    question_prompt = build_prompt(QUESTION, "subject", "title", "script")
...
def test_prompt_says_so_when_no_script_is_stored():
    prompt = build_prompt(QUESTION, "subject", "title", "   ")
...
def test_prompt_forbids_the_engagement_boilerplate():
    ...
    prompt = build_prompt(QUESTION, "s", "t", "script")
    assert "like, subscribe or comment" in prompt


def test_prompt_includes_the_script():
    assert "the whole script here" in build_prompt(QUESTION, "s", "t", "the whole script here")
...
def test_poll_brief_forbids_giving_the_answer_away():
    ...
    prompt = build_prompt(POLL, "s", "t", "script")
